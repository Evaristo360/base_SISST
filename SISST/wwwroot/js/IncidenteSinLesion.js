/*
 *
 * general Incidente Con Lesion functions
 *
 * */
...
function IsGuardadoParcial() {
    $('.GuardarParcial').on('click', function (e) {
        CleanErrors('#formIncidenteSinLesion');
        $('#GuardadoParcial').val(true);

        //partial save only requires Trabajador and Fecha Incidente
        $('#formIncidenteSinLesion').data('validator').settings.ignore = ".guardadoTotal";
        validateForm = true;
        var validationModel = $("#formIncidenteSinLesion").valid();
        if (!validationModel) {
            validateForm = false;
        }
    });
...
function IsGuardadoTotal() {
    $('.GuardarTotal').on('click', function (e) {
        ...
        $('#formIncidenteSinLesion').data('validator').settings.ignore = ".trabajadorSearchVC, .ignoreValidation";
        validateForm = true;
        var validationModel = $("#formIncidenteSinLesion").valid();
        if (!validationModel) {
            validateForm = false;
...
/*
 *
 * init Incidente Con Lesion functions
 *
 * */
...
function InitSelect2() {
    ...
    $("#IdTipoContrato").select2({ width: '100%' });
    $("#RamaActividad").select2({ width: '100%' });
    $("#IdDepartamento").select2({ width: '100%' });
    //$("#IDCategoriaAccidentado").select2({ width: '100%' });

    //descripcion tab
    $("#PartesCuerpoAfectada").select2({ tags: true, width: '100%' });
    $("#TiposContacto").select2({ tags: true, width: '100%' });
    $("#FuentesLesion").select2({ tags: true, width: '100%' });
    $("#SitioDelIncidente").select2({ tags: true, width: '100%' });
    $("#CriteriosLesion").select2({ tags: true, width: '100%' });
...
function CommonIncidenteConLesionSetup() {
    //datos trabajador tab
    //callback for Datos Trabajador searcher
    $('#DatosTrabajadorSearcher.trabajadorSearchVC').on('select2:select', function (e) {
        var trabajadorCallback = GetFullTrabajadorByRPESinLesion;
        LlenarDatosTrabajadorSelect2(e, true, trabajadorCallback);
    });
    //callback for Trabajadores Presencia Evento
    //$('#TrabajadorPresenciaEventoSearcher.trabajadorSearchVC').on('select2:select', function (e) {
    //    var trabajadorCallback = PrepareTrabajadorPresenciaEvento;
    //    LlenarDatosTrabajadorSelect2(e, true, trabajadorCallback);
    //});
    ...
    $('#FechaIngresoPuesto').bind('change', function () {
        CalcularAntiguedadPuesto();
    });
    $('#FechaIngresoCT').bind('change', function () {
...
/*
 *
 * validation Incidente Con Lesion functions
 *
 * */
...
$(document).ready(function () {
    ...
    $('#formIncidenteSinLesion').find("[data-valmsg-summary=true]")
...
//$.validator.addMethod("validaCausas", function (value, element) {
//    try {
//        if (value > 0)
//            return true;
//        else return false;
...
//        return false;
//    }
//}, "Seleccionar alguna causa");


//$.validator.addMethod("validaNecesidad", function (value, element) {
//    try {
//        if (value > 0)
//            return true;
//        else return false;
